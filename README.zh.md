...
一支 linter 加一個 Claude skill，檢查你跟 AI 之間來回的文字。你寫給它的
prompt，查有沒有歧義，因為模型多半不會反問，猜一個意思就直接做下去；它回你
的內容，查有沒有機器腔，因為讀的人一眼就認得出來。
...
**AI 回你的內容**：查句長節奏。上面那張圖就是重點：那段文字每一份字表都會放行，
...
它拒絕從模型產出學習，包括它自己剛改寫完的東西。profile 放在
